...
if len(sys.argv) < 3:
	sys.stderr.write("This program requires MySQL & PostgreSQL hostname " +
                         "and two boolean parameters: 'commit' and 'insert'\n")
	sys.exit(1)

hostname = sys.argv[1]
commit = int(sys.argv[2])
...
rm1 = psycopg2.connect("host=" + hostname + " user=lixa password=passw0rd")

# create a new MySQL connection
# Note: using MySQLdb functions
rm2 = MySQLdb.connect(host=hostname, user="lixa", password="passw0rd", db="lixa")
